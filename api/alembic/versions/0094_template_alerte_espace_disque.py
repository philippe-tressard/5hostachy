...
def upgrade():
    op.execute("""
        INSERT INTO modeleemail (code, libelle, sujet, corps_html, actif, desactivable)
        SELECT 'alerte_espace_disque',
               ...
               1,
               0
        WHERE NOT EXISTS (SELECT 1 FROM modeleemail WHERE code = 'alerte_espace_disque')
    """)


def downgrade():
    op.execute("DELETE FROM modeleemail WHERE code = 'alerte_espace_disque'")
