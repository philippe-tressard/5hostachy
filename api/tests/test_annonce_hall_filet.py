...
La CI installe pango/cairo — c'est là qu'il fait barrage.
"""
...
def _rectangles_peints(pdf: str) -> list[tuple[tuple[float, float, float], float, float, float, float]]:
...
def test_un_seul_filet_dore_est_VISIBLE(format_effectif: str):
    ...
    assert len(visibles) == 1, (
        ...
        "au lieu d'un seul :\n  "
        + "\n  ".join(f"x={x:.2f} y={y:.2f} l={l:.2f} h={h:.2f}" for _, x, y, l, h in visibles)
