...
ANGLAIS = {
    "user", "users", "token", "tokens", "expires", "last", "seen", "login", "logout",
    "register", "refresh", "upload", "stats", "match", "search", "count", "active",
    "event", "daily", "monthly", "scheduled", "log", "logs", "test", "status", "reset",
    "password", "me", "telemetry",
}

#: Tables anglaises existantes au 24/09/2026 — à renommer au fil de l'eau.
TABLES_FIGEES = {
    "email_verification_token", "password_reset_token", "refresh_token",
    "telemetry_daily", "telemetry_event", "telemetry_monthly", "user_lot",
    "whatsapp_log", "whatsapp_scheduled",
}

#: Routes existantes portant un mot anglais au 24/09/2026 — même régime.
ROUTES_FIGEES = {
    "/acces/admin/imports-vigik/auto-match", "/acces/admin/imports-vigik/stats",
    "/acces/admin/imports-vigik/upload", "/acces/admin/imports/auto-match",
    "/acces/admin/imports/stats", "/acces/admin/imports/upload",
    "/admin/audit/user-lots", "/admin/comptes/{user_id}/traiter",
    "/admin/me/accueil-arrivant", "/admin/telemetry/agreger",
    "/admin/telemetry/historique", "/admin/user-lots/{user_lot_id}",
    "/admin/utilisateurs/{user_id}", "/admin/utilisateurs/{user_id}/accueil-arrivant",
    "/admin/utilisateurs/{user_id}/ajouter-role", "/admin/utilisateurs/{user_id}/auto-match",
    "/admin/utilisateurs/{user_id}/ban-communaute", "/admin/utilisateurs/{user_id}/retirer-role",
    "/auth/change-password", "/auth/login", "/auth/logout", "/auth/me",
    "/auth/me/demande-modification", "/auth/me/demandes-modification",
    "/auth/me/opt-out-telemetrie", "/auth/me/telemetrie", "/auth/refresh", "/auth/register",
    "/bailleur/search-locataire", "/config/imap-test", "/config/llm-test", "/config/smtp-test",
    "/config/whatsapp-logs", "/config/whatsapp-scheduled", "/config/whatsapp-scheduled/{item_id}",
    "/config/whatsapp-status", "/config/whatsapp-test", "/lots/admin/imports/auto-match",
    "/lots/admin/imports/stats", "/lots/admin/imports/upload", "/signalements/count",
    "/telemetry/collect", "/telemetry/dashboard", "/telemetry/users-active",
}
...
def test_les_tags_suivent_la_convention(schema):
    tags = {
        t for chemin in schema["paths"].values() for op in chemin.values()
