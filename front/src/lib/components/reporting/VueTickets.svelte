...
<script lang="ts">
	...
	$: reportTicketCategories = Object.entries(
		reportTicketSource.reduce((acc: Record<string, { total: number; ouverts: number }>, t) => {
	...
	$: reportTicketBuildings = Object.entries(
		reportTicketSource.reduce((acc: Record<string, { total: number; ouverts: number }>, t) => {
		...
		.map(([batiment, data]) => ({ batiment, ...data }))
		.sort((a, b) => b.total - a.total);
</script>
...
<div class="reporting-toolbar no-print" style="margin-top:0;margin-bottom:1rem">
	<div class="reporting-switch">
		<Pastille active={reportPeriodDays === 30} on:click={() => (reportPeriodDays = 30)}
			>30 jours</Pastille
		>
		<Pastille active={reportPeriodDays === 90} on:click={() => (reportPeriodDays = 90)}
			>90 jours</Pastille
		>
		<Pastille active={reportPeriodDays === 365} on:click={() => (reportPeriodDays = 365)}
			>12 mois</Pastille
		>
	</div>
</div>
...
<div class="kpi-row" style="margin-bottom:1rem">
	...
	<div class="kpi-card">
		<div class="kpi-value">{reportTicketBuildings.length}</div>
		<div class="kpi-label">Périmètres / bâtiments touchés</div>
	</div>
</div>
