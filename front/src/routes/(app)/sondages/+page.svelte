...
{:else}
	...
	<div class="tabs" role="tablist" style="margin-bottom:1.5rem">
		<button
			role="tab"
			class:active={activeTab === 'sondages'}
			on:click={() => (activeTab = 'sondages')}
		>
						...
						<div class="moderation-item">
							...
							<div class="moderation-actions">
								<button
									...
									on:click={() => resoudreSignalement(sig.id, 'traite')}>✓ Marquer traité</button
								>
								<button
									class="btn btn-sm btn-outline"
									on:click={() => resoudreSignalement(sig.id, 'rejete')}>Ignorer</button
								>
							</div>
						</div>
					{/each}
