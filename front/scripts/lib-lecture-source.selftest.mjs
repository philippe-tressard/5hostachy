...
export function selftest() {
	...
	verifier(
		...
		corpsDesTables(TABLES, /export const (LIBELLE_TACHE)\b/g).flatMap((c) =>
			valeursDeclarees(c, '[a-z_]+'),
		),
	);
	verifier(
		'accolade jamais refermée → aucun bloc',
		[],
		corpsDesTables('export const LIBELLE_X = {', /export const (LIBELLE_[A-Z_]+)\b/g),
	);
	...
	verifier(
		'<ModaleConfirmation> ne compte pas',
		false,
		emploieComposant('<ModaleConfirmation>', 'Modale'),
	);
	...
	verifier(
		'</button> coupé par le formatage',
		20,
		baliseFermante('<button>Oui</button\n>', 'button').fin,
	);
