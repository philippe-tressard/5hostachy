...
# ── Rotation d'UN journal, sans jamais délier son inode ───────────────────────
#  ⚠️ `tail > tmp && mv tmp log` REMPLACE l'inode. Or ce fichier est tenu OUVERT :
#  ce script lui-même y écrit via le `>> /var/log/hostachy-maintenance.log` du
#  cron, health-watch.sh tourne toutes les 5 min et check-reliability.sh toutes
#  les 15 — la rotation dure plus d'une minute et croise donc leurs exécutions.
#  Après le `mv`, leurs écritures partent dans un inode ORPHELIN : invisibles, et
#  perdues à la fin du process. C'est la règle d'or anti-corruption DB
#  (`standards/06-donnees-et-integrite.md` §1) appliquée à un journal — ne jamais
#  délier un fichier qu'un process tient ouvert.
#
#  Constaté le 09/08/2026 : hostachy-maintenance.log s'arrête net à 03:02:55, à la
#  position alphabétique qui précède exactement son propre nom. Tout ce qui suit —
#  la fin du ménage, l'envoi du rapport, et une éventuelle erreur — est illisible.
#  Le défaut avait donc détruit la trace de ses propres conséquences.
#
...
#  `chown` de rattrapage plus bas — le `mv` repassait le log root-owned et coupait
#  SILENCIEUSEMENT le cron utilisateur auto-deploy, découvert le 15/07/2026).
#  Les fd du cron sont en mode APPEND (`>>`) : après troncature, la suite s'écrit
#  à la nouvelle fin, sans trou creux.
...
hygiene_locale() {
    ...
    for LOGFILE in /var/log/hostachy-*.log; do
        [ -f "$LOGFILE" ] || continue
        TRIMMED=$(roter_log "$LOGFILE" "$LOG_KEEP_LINES")
    ...
    # Le log auto-deploy est écrit par un cron UTILISATEUR (ptressard), pas root.
    # La rotation ne le déproprie plus (roter_log conserve l'inode, donc le
    # propriétaire) — mais un log déjà repassé root-owned par une rotation
    # ANTÉRIEURE le resterait, et le cron user cesserait SILENCIEUSEMENT d'écrire
    # (cause du non-déploiement de rpi1 découvert le 15/07). Ce chown répare
    # désormais un état hérité, il ne compense plus un défaut actif.
...
#  ce que font le cron de ce script, health-watch.sh et check-reliability.sh —
